'use client';

import { motion } from 'framer-motion';

const primaryGold = '#c9a962';

const socialLinks = [
  {
    id: 'linkedin',
    name: 'LinkedIn',
    href: 'https://www.linkedin.com/in/dikshyagiri/',
    description: 'Connect with me professionally',
    icon: (
      <svg width="40" height="40" viewBox="0 0 24 24" fill="currentColor" className="text-primary">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
  {
    id: 'github',
    name: 'GitHub',
    href: 'https://github.com/dikshya48',
    description: 'Explore my code and projects',
    icon: (
      <svg width="40" height="40" viewBox="0 0 24 24" fill="currentColor" className="text-primary">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
      </svg>
    ),
  },
];

const SocialLinksSection = () => {
  return (
    <section className="relative py-24 px-6">
      <div className="max-w-4xl mx-auto">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="text-center mb-12"
        >
          <h2 className="font-fraunces text-3xl md:text-4xl font-semibold text-foreground mb-3">
            Connect With Me
          </h2>
          <p className="text-muted-foreground text-lg max-w-md mx-auto">
            Find me on LinkedIn and GitHub
          </p>
        </motion.div>

        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="flex flex-col md:flex-row gap-6 md:gap-8 items-stretch"
        >
          {socialLinks.map((link, i) => (
            <motion.a
              key={link.id}
              href={link.href}
              target="_blank"
              rel="noopener noreferrer"
              initial={{ opacity: 0, x: i === 0 ? -30 : 30 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              animate={{
                y: [0, i === 0 ? -6 : 6, 0],
                boxShadow: [
                  `0 0 20px ${primaryGold}40, 0 0 40px ${primaryGold}20`,
                  `0 0 28px ${primaryGold}60, 0 0 56px ${primaryGold}30`,
                  `0 0 20px ${primaryGold}40, 0 0 40px ${primaryGold}20`,
                ],
              }}
              transition={{
                y: { duration: 4, repeat: Infinity, ease: 'easeInOut', delay: i * 0.3 },
                boxShadow: { duration: 2.5, repeat: Infinity, ease: 'easeInOut', delay: i * 0.2 },
              }}
              whileHover={{
                scale: 1.04,
                boxShadow: `0 0 35px ${primaryGold}80, 0 0 70px ${primaryGold}40`,
                transition: { duration: 0.3 },
              }}
              className="relative flex-1 overflow-hidden rounded-2xl border-2 border-primary/60 bg-white/[0.02] backdrop-blur-xl p-8 flex flex-col items-center justify-center min-h-[180px] text-center no-underline group"
            >
              <div className="mb-4 group-hover:scale-110 transition-transform">
                {link.icon}
              </div>
              <h3 className="font-fraunces text-2xl font-semibold text-foreground mb-2" style={{ textShadow: `0 0 12px ${primaryGold}60` }}>
                {link.name}
              </h3>
              <p className="text-muted-foreground text-lg mb-4">{link.description}</p>
              <span className="inline-flex items-center gap-2 text-primary font-medium text-lg group-hover:gap-3 transition-all">
                Visit Profile
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6M15 3h6v6M10 14L21 3" />
                </svg>
              </span>
            </motion.a>
          ))}
        </motion.div>
      </div>
    </section>
  );
};

export default SocialLinksSection;
